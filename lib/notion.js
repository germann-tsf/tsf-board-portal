...
// Resolve committee relation IDs to names by fetching each related page directly
async function resolveCommitteeNames(committeeIds) {
  const map = {}
    ...
    try {
      ...
      for (const key of Object.keys(page.properties)) {
        if (page.properties[key].type === 'title') {
          map[id] = getRichTextValue(page.properties[key].title)
          break
        }
      }
    } catch (error) {
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const page of response.results) {
        ...
        const committeeIds = getPropertyValue(props['Committees (internal)']) || []

        committeeIds.forEach(id => allCommitteeIds.add(id))
    ...
    // Resolve committee names from relation IDs (without querying a separate DB)
    const committeeMap = await resolveCommitteeNames([...allCommitteeIds])
    for (const member of members) {
      member.committees = (member.committeeIds || [])
        .map(id => committeeMap[id])
        .filter(Boolean)
      delete member.committeeIds
    }
