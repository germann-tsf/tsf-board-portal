...
export async function debugBoardMembers() {
  const diag = { dbId: process.env.NOTION_MEMBERS_DB }
  // Test 1: search for the database by name to find correct ID
  try {
    const search = await notion.search({ query: 'Board Member', filter: { property: 'object', value: 'database' } })
    diag.searchResults = search.results.map(r => ({ id: r.id, title: r.title?.[0]?.plain_text }))
  } catch (e) {
    diag.searchError = e.message
  }
  // Test 2: try env var ID
  try {
    const db = await notion.databases.retrieve({ database_id: process.env.NOTION_MEMBERS_DB })
    diag.envIdTitle = db.title?.[0]?.plain_text
  } catch (e) {
    diag.envIdError = { message: e.message, code: e.code, status: e.status }
  }
  // Test 3: query with env var ID
  try {
    const q = await notion.databases.query({ database_id: process.env.NOTION_MEMBERS_DB, page_size: 2 })
    diag.envIdQueryCount = q.results.length
    diag.envIdQueryHasMore = q.has_more
  } catch (e) {
    diag.envIdQueryError = { message: e.message, code: e.code, status: e.status }
  }
  return diag
}
