...
// Diagnostic: query without filter to check if integration has access at all
export async function debugBoardMembers() {
  const diag = { dbId: process.env.NOTION_MEMBERS_DB, hasApiKey: !!process.env.NOTION_API_KEY }
  try {
    // Test 1: retrieve database schema
    const db = await notion.databases.retrieve({ database_id: process.env.NOTION_MEMBERS_DB })
    diag.dbTitle = db.title?.[0]?.plain_text || 'no title'
    diag.statusPropType = db.properties['Status']?.type || 'NOT FOUND'
    diag.propNames = Object.keys(db.properties)
  } catch (e) {
    diag.dbRetrieveError = { message: e.message, code: e.code, status: e.status }
  }
  try {
    // Test 2: query without filter
    const raw = await notion.databases.query({ database_id: process.env.NOTION_MEMBERS_DB, page_size: 3 })
    diag.unfilteredCount = raw.results.length
    diag.unfilteredHasMore = raw.has_more
    diag.firstPageStatus = raw.results[0]?.properties?.['Status']?.status?.name || 'NONE'
    diag.firstPageName = raw.results[0]?.properties?.['Board Member']?.title?.[0]?.plain_text || 'NONE'
  } catch (e) {
    diag.unfilteredError = { message: e.message, code: e.code, status: e.status }
  }
  try {
    // Test 3: query with filter
    const filtered = await notion.databases.query({
      database_id: process.env.NOTION_MEMBERS_DB,
      filter: { property: 'Status', status: { equals: 'Current Board Member' } },
      page_size: 3,
    })
    diag.filteredCount = filtered.results.length
    diag.filteredHasMore = filtered.has_more
  } catch (e) {
    diag.filteredError = { message: e.message, code: e.code, status: e.status }
  }
  // Test 4: try alternate DB ID (page ID vs collection ID)
  const altDbId = '22984a2d-4690-808f-ba72-e207a21ea592'
  try {
    const altRaw = await notion.databases.query({ database_id: altDbId, page_size: 3 })
    diag.altDbIdCount = altRaw.results.length
    diag.altDbIdHasMore = altRaw.has_more
    diag.altFirstName = altRaw.results[0]?.properties?.['Board Member']?.title?.[0]?.plain_text || 'NONE'
  } catch (e) {
    diag.altDbIdError = { message: e.message, code: e.code, status: e.status }
  }
  return diag
}
